...
%-- Parameters
%------------------------------
filename = '../../vhdl/spi/src/spi_master.vhd';
...
%------------------------------
wrapper_filename= [value_entity_name '_wrapper.vhd'];
wrapper_entity_name = [value_entity_name '_wrapper'];
wrapper_clock_name = value_port_name(value_clock_is,:);
if value_reset_is>0
  wrapper_reset_name = value_port_name(value_reset_is,:);
end
...
fprintf(fid,"\n");
fprintf(fid,"entity %s is\n",strtrim(wrapper_entity_name));
fprintf(fid,"port (\n");
for k=1:size_ports_data_row(1)
  if k==size_ports_data_row(1)
    fprintf(fid,"    %s : %s %s\n", value_port_name(k,:), value_port_direction(k,:), strtrim(value_port_type(k,:)));
  else
    fprintf(fid,"    %s : %s %s;\n", value_port_name(k,:), value_port_direction(k,:), strtrim(value_port_type(k,:)));
  end;
end
...
%wrap in signals
fprintf(fid,"\n");
fprintf(fid,"--wrap in signals\n");
for k=1:size_ports_data_row(1)
 %In signals
  if (value_reset_is>0 & k==value_reset_is)
    %do nothing
  elseif (value_clock_is>0 & k==value_clock_is)
    %do nothing
  else
    tmp = strcmp('in ',value_port_direction(k,:));
    if tmp==1
      fprintf(fid,"signal wrap_in_%s : %s;\n",strtrim(value_port_name(k,:)),strtrim(value_port_type(k,:)));
    end;
  end;
end
...
for k=1:size_ports_data_row(1)
 %In signals
  ...
  elseif (value_clock_is>0 & k==value_clock_is)
    ...
    tmp = strcmp('out',value_port_direction(k,:));
    if tmp==1
      fprintf(fid,"signal wrap_out_%s : %s;\n",strtrim(value_port_name(k,:)),strtrim(value_port_type(k,:)));
    end;
  end;
end
...
fprintf(fid,"\n");
fprintf(fid,"-----------------------------------------\n");
fprintf(fid,"--Wrap process\n");
fprintf(fid,"-----------------------------------------\n");
fprintf(fid,'wrap_pr: process(%s)          \n',strtrim(value_port_name(value_clock_is,:)));
fprintf(fid,'begin                            \n');
fprintf(fid,'    if rising_edge(%s) then      \n',strtrim(value_port_name(value_clock_is,:)));
%Here insert the connection between ports
