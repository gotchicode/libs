...
%Reset file out of the loop
fid=fopen('cic_loop_gen.txt','w');
fprintf(fid,'Fout; R; N; Fin; Cutoff; min_6dB; perc.; min_0_02dB; perc.;cic_gain_dB;nb_taps;\n');
fclose(fid)
...
%CIC Parameters
M = 1;
%% Loop param R = 8;  % decimation or interpolation ratio
...
fid=fopen('cic_loop_gen.txt','a');
fprintf(fid,'%f;',fsamp);
fprintf(fid,'%f;',R);
fprintf(fid,'%f;',N);
fprintf(fid,'%f;',fsamp/R);
fprintf(fid,'%f;',cut_off_freq);
fprintf(fid,'%f;',min_6dB_freq);
fprintf(fid,'%f;',min_6dB_freq/cut_off_freq*100);
fprintf(fid,'%f;',min_0_02dB_freq);
fprintf(fid,'%f;',min_0_02dB_freq/cut_off_freq*100);
fprintf(fid,'%f;',20*log10(max_h_cic_resp_lin));
fprintf(fid,'%f;',nb_taps);
fprintf(fid,'\n');
fclose(fid);
